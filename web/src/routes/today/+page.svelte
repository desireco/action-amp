<script lang="ts">
  ...
  function pickTask(task: TaskLensListRowDto) {
    void window.location.assign(`/today/${encodeURIComponent(task.permalink ?? task.id)}`);
  }
</script>
...
<section class="aa-today" aria-label="Today">
  <header class="aa-today__hero">
    ...
    <div class="aa-today__hero-links">
      <CountLinkButton label="This week" count={weekCount} to="/week" />
      <CountLinkButton label="Upcoming" count={lists.appData ? upcomingCount : undefined} to="/upcoming" />
    </div>
  </header>
  ...
  {:else if isEmpty}
    ...
    >
      ...
      {#snippet action()}
        {#if upcomingCount > 0}
          <a href="/upcoming" class="aa-btn aa-btn--secondary">See upcoming {upcomingCount}</a>
        {/if}
      {/snippet}
    </ListEmpty>
  {:else}
        ...
        <GroupedList groups={doneGroups} headingLevel={3}>
          {#snippet renderItem(item)}
            ...
            <TaskRow task={task} showLens={showLensPill} muted>
              ...
              <a class="aa-btn aa-btn--ghost" href="/tasks/{task.permalink ?? task.id}">Open</a>
            </TaskRow>
          {/snippet}
        </GroupedList>
      {/if}
