 ...
 *
 *   API:  cd api && DATABASE_URL=postgresql://jake@localhost:5432/actionamp_dev \
 *           NODE_ENV=development bun --hot src/index.ts
 *   Web:  cd web && bunx vite dev --port 5174
 *
...
const API_START = `cd api && DATABASE_URL=postgresql://jake@localhost:5432/actionamp_dev NODE_ENV=development bun --hot src/index.ts`;
const WEB_START = `cd web && bunx vite dev --port 5174`;
...
export default async function globalSetup() {
  ...
  for (const seed of ["seed.ts", "seed-s4.ts", "seed-inbox.ts", "seed-projects.ts", "seed-lenses.ts", "seed-admin.ts", "seed-billing.ts"]) {
