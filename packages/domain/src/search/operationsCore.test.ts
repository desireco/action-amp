...
// Ported verbatim from webapp/src/search/operationsCore.test.ts (S9) — only
// the import specifier changed. The mock entities REPLACE the seam delegates,
// so the exact call shapes (3 passes, take: 11, projected selects) assert the
// same way they did against Wasp's context.entities.

function entities(): Parameters<typeof searchSiteData>[0] {
  // SAFETY: the search core's entities slice is the webapp's loose
  // `Record<string, any>` (Prisma cannot type a heterogeneous relevance
  // read); the single cast here keeps the ported mocks + assertions unchanged.
  return {
  ...
  } as Parameters<typeof searchSiteData>[0];
}
...
describe("searchSiteData", () => {
  ...
  it("ranks the complete match set so an older exact match is not hidden", async () => {
    ...
    db.Task.findMany.mockImplementation(async (args: {
      where: {
        OR?: Array<Record<string, { equals?: string; startsWith?: string }>>;
      };
    }) => {
    ...
    expect(
      db.Task.findMany.mock.calls.every(
        ([args]: [{ take: number }]) => args.take === 11,
      ),
    ).toBe(true);
