/**
 * Pure sitewide-search core — ported from webapp/src/search/operationsCore.ts
 * (S9; the parity checklist lives in
 * packages/contract/src/s9-search-resources/README.md).
 *
 * Wasp wrapper owned auth + entitlement; this module owns tenant-scoped
 * database reads, shaping, ranking, snippets, and limits. Bodies are verbatim
 * against the webapp original; the differences are type-level only (seam
 * types instead of `@prisma/client`, `.js` import specifiers, the entities
 * slice instead of Wasp's context.entities).
 *
 * The delegate-call boundary stays loose (`any` helpers, as in the webapp
 * original): Prisma cannot type a heterogeneous multi-model relevance read,
 * and the exact call shapes (3 bounded passes per kind, take: 11, projected
 * relations) are pinned by the ported unit tests instead.
 */

// Prisma delegates from the seam (`createEntities(db)`) or Vitest spies.
