// Ported verbatim from webapp/src/resources/operationsCore.test.ts (S9) —
// import specifier only. The guards' SIMPLE_LIST rejection lives in
// guards.test.ts (webapp had it in the op layer).
import { describe, expect, it, vi } from "vitest";
import { createResourceCore } from "./operationsCore.js";
...
describe("createResourceCore", () => {
  it("requires an owned project and trims the title", async () => {
    ...
    await createResourceCore(db as Parameters<typeof createResourceCore>[0], {
      userId: "user-1",
      projectId: "project-1",
  ...
  it("rejects an empty title and an unknown project", async () => {
    ...
    db.Project.findFirst.mockResolvedValue(standardProject);
    await expect(
      createResourceCore(db as Parameters<typeof createResourceCore>[0], { userId: "user-1", projectId: "project-1", title: "   " }),
    ).rejects.toThrow(/cannot be empty/i);

    db.Project.findFirst.mockResolvedValue(null);
    await expect(
      createResourceCore(db as Parameters<typeof createResourceCore>[0], { userId: "user-1", projectId: "gone", title: "x" }),
  ...
  it("rejects non-image attachments and oversized images", async () => {
    ...
    db.Project.findFirst.mockResolvedValue(standardProject);
    await expect(
      createResourceCore(db as Parameters<typeof createResourceCore>[0], {
        userId: "user-1",
        projectId: "project-1",
        title: "x",
