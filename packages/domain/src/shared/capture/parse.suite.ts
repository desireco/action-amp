/**
 * The capture-parser contract suite — the exact same describe tree runs
 * against BOTH Temporal bindings (#15): parse.test.ts feeds it the real Bun
 * global; parse.browser.test.ts clears the global, installs the mini
 * browser namespace, and runs it again. Any drift the dual-binding
 * architecture could introduce shows up here first.
 *
...
import type { parseCapture as parseCaptureFn } from "./parse.js";

export function parserSuite(parseCapture: typeof parseCaptureFn): void {
