/**
 * Browser-side Temporal binding for @actionamp/domain (#15).
 *
 * The domain package binds Temporal once per realm: `temporal.ts` reads
 * `globalThis.Temporal` at module evaluation. On Bun the native global is
 * already there; in the browser this module installs a MINIMAL Temporal onto
 * `globalThis` first — only the surface the capture parser + the small
 * date-format helpers use — when the platform lacks one. Where a real
 * Temporal exists it is left untouched, so a future Chromium that ships the
 * global simply switches both realms to the real thing (deleting this file
 * then becomes a one-line follow-up). Each realm has exactly one flavor, so
 * the mixed-internal-slots hazard cannot arise by construction.
 *
