...
export const analyticsSession = pgTable("AnalyticsSession", {
	...
	lastSeenAt: timestamp({ precision: 3, mode: 'date' }).notNull(),
	referrerHost: text(),
	utmSource: text(),
	utmMedium: text(),
	utmCampaign: text(),
...
export const inboxItem = pgTable("InboxItem", {
	...
	parsedLensId: text(),
	parsedScheduledDate: date({ mode: 'date' }),
	parsedSnoozedUntil: timestamp({ precision: 3, withTimezone: true, mode: 'date' }),
}, (table) => [
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const user = pgTable("User", {
	...
	planRenewsAt: timestamp({ precision: 3, mode: 'date' }),
	stripeCustomerId: text(),
	fullName: text().notNull(),
	preferredName: text(),
	hasSeenOnboarding: boolean().default(false).notNull(),
	lastTodayRolloverAt: timestamp({ precision: 3, mode: 'date' }),
	...
	lastDailyReminderAt: timestamp({ precision: 3, mode: 'date' }),
