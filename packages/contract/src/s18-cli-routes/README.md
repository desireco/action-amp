...
| `/api/cli/task/sweep` | POST | `{ olderThanDays?, lensId?, apply? }` — **new-stack addition, no webapp ancestor**: bulk-park stale Upcoming rows (untouched `olderThanDays`, default 30; future snoozes skipped) to Someday. Dry run unless `apply: true` | `{ dryRun: boolean, tasks: TaskLensListRow[] }` | 400 `"olderThanDays must be a whole number between 1 and 365."`; explicit lensId → 404 lens / 402 lens violation; 500 `"Could not sweep tasks."` |
