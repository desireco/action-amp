...
export const ANALYTICS_EVENTS = [
  "LANDING_VIEW", "PRICING_VIEW", "FOUNDING_VIEW", "SIGNUP_STARTED",
  "SIGNUP_COMPLETED", "APP_OPENED", "ONBOARDING_COMPLETED", "CAPTURE_CREATED",
  "TRIAGE_COMPLETED", "FOCUS_STARTED", "TASK_COMPLETED", "CHECKOUT_STARTED",
...
function validateMetadata(metadata: AnalyticsEventInput["metadata"]): Record<string, string | number | boolean | null> | null {
...
export async function recordAnalyticsEventCore(
  ...
  if (!ANALYTICS_EVENTS.includes(input.name)) throw new Error("Unknown analytics event.");
  const visitorId = clean(input.visitorId, 80);
  if (!visitorId || !/^[a-zA-Z0-9_-]+$/.test(visitorId)) throw new Error("Invalid analytics visitor id.");
  ...
  let session = userId && visitorId.startsWith("user_")
    ...
    : null;
  if (session) {
    await entities.AnalyticsSession.update({ where: { id: session.id }, data: { lastSeenAt: new Date() } });
  } else {
...
export async function getFunnelStatsCore(entities: Entities, range: FunnelRange): Promise<FunnelStats> {
  const now = Date.now();
  const sinceDate = range === "all" ? null : new Date(now - (range === "7d" ? 7 : 30) * 24 * 60 * 60 * 1000);
  const sessions = (await entities.AnalyticsSession.findMany({
    where: sinceDate ? { firstSeenAt: { gte: sinceDate } } : undefined,
    select: {
      id: true, referrerHost: true, utmSource: true, utmMedium: true, utmCampaign: true,
      events: {
  ...
  const names: AnalyticsEventName[] = [
    "LANDING_VIEW", "SIGNUP_COMPLETED", "APP_OPENED", "CAPTURE_CREATED",
    "TRIAGE_COMPLETED", "CHECKOUT_STARTED", "PAYMENT_CONFIRMED",
  ];
  const counts = names.map((name) => sessions.filter((s: any) => s.events.some((e: any) => e.name === name)).length);
  const landingCount = counts[0] ?? 0;
  const funnel = names.map((name, i) => ({
    name,
    count: counts[i] ?? 0,
    fromPreviousPct: i === 0 ? null : counts[i - 1] ? Math.round(((counts[i] ?? 0) / counts[i - 1]) * 1000) / 10 : null,
    fromLandingPct: i === 0 ? 100 : landingCount ? Math.round(((counts[i] ?? 0) / landingCount) * 1000) / 10 : null,
  }));
  ...
  for (const session of sessions as any[]) {
    ...
    row.sessions += 1;
    if (session.events.some((e: any) => e.name === "SIGNUP_COMPLETED")) row.signups += 1;
    if (session.events.some((e: any) => e.name === "CHECKOUT_STARTED")) row.checkouts += 1;
    if (session.events.some((e: any) => e.name === "PAYMENT_CONFIRMED")) row.payments += 1;
  ...
  const cohortUsers = Array.from(appOpensByUser.values()).map((dates) => dates.sort((a, b) => a.getTime() - b.getTime()));
  const d1Eligible = cohortUsers.filter(([first]) => first && first.getTime() + 24 * 60 * 60 * 1000 <= now);
  const d7Eligible = cohortUsers.filter(([first]) => first && first.getTime() + 7 * 24 * 60 * 60 * 1000 <= now);
  const returnedAt = (dates: Date[], days: number) => {
    const first = dates[0];
    return !!first && dates.some((date) => date.getTime() >= first.getTime() + days * 24 * 60 * 60 * 1000);
  };
  const retention = {
    d1Pct: d1Eligible.length ? Math.round((d1Eligible.filter((dates) => returnedAt(dates, 1)).length / d1Eligible.length) * 1000) / 10 : null,
    d7Pct: d7Eligible.length ? Math.round((d7Eligible.filter((dates) => returnedAt(dates, 7)).length / d7Eligible.length) * 1000) / 10 : null,
    note: d1Eligible.length || d7Eligible.length ? undefined : "Not enough elapsed time for a return cohort in this range.",
  };
  ...
  return {
    ...
    sources: Array.from(sourceMap, ([source, row]) => ({
      source, ...row, conversionPct: row.sessions ? Math.round((row.payments / row.sessions) * 1000) / 10 : null,
