...
function asAnalytics(entities: MockEntities) {
  const spies = {
  ...
  return spies as AnalyticsEventEntities;
}

function asFunnel(entities: MockEntities) {
  const spies = { AnalyticsSession: entities.AnalyticsSession };
  // SAFETY: EntitySpy vi.fn()s satisfy the delegate slice at runtime.
  return spies as FunnelEntities;
}
...
describe("recordAnalyticsEventCore", () => {
  it("validates metadata and records a typed event in the visitor session", async () => {
    ...
    entities.AnalyticsSession.upsert.mockResolvedValue({
      id: "session-1",
      userId: null,
    });
    entities.AnalyticsEvent.create.mockResolvedValue({ id: "event-1" });

    const result = await recordAnalyticsEventCore(asAnalytics(entities), {
      name: "LANDING_VIEW",
    ...
    expect(entities.AnalyticsEvent.create).toHaveBeenCalledWith(
  ...
  it("deduplicates one-time events for a known user", async () => {
    const { entities } = mockContext();
    entities.AnalyticsSession.findFirst.mockResolvedValue({
      id: "session-1",
      userId: "user-1",
    });
    entities.AnalyticsEvent.findFirst.mockResolvedValue({ id: "existing" });

    const result = await recordAnalyticsEventCore(
      asAnalytics(entities),
      {
  ...
  it("groups funnel steps and unknown acquisition sources without PII", async () => {
    ...
    entities.AnalyticsSession.findMany.mockResolvedValue([
      ...
      },
      {
        ...
        utmMedium: null,
        utmCampaign: null,
        events: [{ name: "LANDING_VIEW", userId: null, occurredAt: day }],
      },
    ]);

    const result = await getFunnelStatsCore(asFunnel(entities), "all");
