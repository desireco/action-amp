...
export const sendDailyTodayReminder: SendDailyTodayReminder<DailyReminderArgs, { sent: number }> = async (_args, context) => {
  ...
  for (const user of users) {
    ...
    const todayCount = await context.entities.Task.count({ where: { userId: user.id, status: "TODAY", isDone: false } });
    const body = todayCount > 0
      ? `${todayCount} task${todayCount === 1 ? "" : "s"} on Today. Pick one.`
      : "Today is clear. Choose what matters.";
