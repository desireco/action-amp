...
If the decision is the cost, then the surface that presents the decision is where the leverage is. And almost every app presents it the same way: as the full list, sorted by date or priority, all of it visible, all of it quietly competing for the next slot in your attention.
...
The list is the problem, not the answer. The answer is to stop opening to it.
