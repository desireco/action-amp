...
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| (bare) `actionamp`                                                     | Login status (who you are, or that you're not logged in) + the command reference                                             |
| `login [--dev]`                                                        | Browser login (default: prod; `--dev` → localhost)                                                                           |
...
| `ritual list [--lens-id <id>]`                                         | List rituals (default: the active lens); `(g)/(b)` mark guidance/benefit                                                     |
| `ritual today`                                                         | Today's due rituals with checked state — the `check` targets                                                                 |
| `ritual create <name> [--interval morning\|midday\|evening] [--cadence daily\|weekdays\|weekly\|interval] [--weekday mon..sun] [--every <days>] [--guidance <text>] [--benefit <text>] [--goal <id>]` | Create a ritual (defaults: morning, every day, Me lens)                                                                      |
| `ritual show <id>`                                                     | A ritual + its checked days, newest first (mood + note)                                                                       |
| `ritual update <id> [--name] [--interval] [--cadence] [--weekday] [--every] [--guidance] [--benefit] [--goal <id>]` | Edit; `--guidance ""` clears (`--benefit`/`--goal` too)                                                                      |
...
| `ritual delete <id>`                                                   | Hard-delete an ARCHIVED ritual (its history goes with it; active ones must be archived first)                                 |                                                                                |
...
1. CLI opens the browser to `/cli/login?state=<one-time nonce>`.
2. The page (session-authed, explicit confirm) mints an `ApiKey` via the
   `mintCliToken` action and files it server-side under that nonce.
3. The CLI polls `GET /api/auth/cli-login-challenge?state=…` and picks the
   token up (single-use, 10-minute window) — no localhost listener, nothing
   local to run. (CLIs ≤0.1.0 passed a localhost `callback` param instead;
   the page still honors that legacy handoff.)
4. CLI validates the token against `/api/cli/whoami`, stores it in
