...
export const POST: APIRoute = createAPIRoute(async ({ request, locals }) => {
  ...
  // Create the updates file path relative to data root
  const updatesFilePathPrefix = path.join('areas', area, project, `${filename}.updates.json`);

  // Load existing updates
  let updates = [];
  try {
    const existingContent = await fsApi.readFile(updatesFilePathPrefix, currentUser);
  ...
  await fsApi.writeFile(updatesFilePathPrefix, JSON.stringify(updates, null, 2), currentUser);
