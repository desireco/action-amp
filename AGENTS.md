...
| The platform switch off Wasp — roadmap, status, how we work | `docs/plans/PLATFORM-SWITCH.md` (home; links the v3 plan, goal set, spike, dispatch files) |
| Switch goal set + model dispatch rules | `docs/plans/2026-08-31-platform-switch-goals.md` |
...
- **Single source of truth:** `web/src/lib/tokens.css` owns every color, type
  ...
  (`npm run storybook`), and `npm run design` opens the Open Design app
  mapped to `web/` + `docs/` (the OpenDesign daemon must be running).
